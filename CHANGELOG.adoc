...
* Resolve dependency advisories by disabling unused Nickel Markdown, documentation, REPL and formatter features, updating `crossbeam-epoch`, and overriding Nickel's vector crate to use `imbl-sized-chunks` 0.2.0 (removing the affected `bitmaps` dependency). See `crates/nickel-lang-vector/README.adoc` for the upstream override and removal plan.
* Raise the declared minimum Rust version and pinned development toolchain to 1.90, as required by the resolved Nickel dependencies.

==== Fixed

* Restore `--locked` compatibility of `Cargo.lock`: prune `futures-macro` 0.3.31, `simd-adler32` 0.3.8 and `tokio-macros` 2.6.0 plus eight dependency edges (across seven packages) that were only reachable through dev-dependencies of non-member packages, which the resolver does not unify into the lock.
